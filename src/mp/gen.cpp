            ...
            ForEachMethod(interface, [&] (const capnp::InterfaceSchema& method_interface, const capnp::InterfaceSchema::Method& method) {
                ...
                if (has_extra) {
                    ...
                    for (const auto annotation : method.getProto().getAnnotations()) {
                        ...
                        Field field;
                        field.extra = true;
                        // The annotation text may be empty, so synthesize a name for the C++ parameter.
                        const kj::StringPtr name{annotation.getValue().getText()};
                        field.extra_name = name.size() > 0 ? kj::str(name) : kj::str("unnamed_extra_param", extra_count);
                        field.args = 1;
                        ++extra_count;
                        pos = std::next(fields.fields.insert(pos, std::move(field)));
                    }
                }
